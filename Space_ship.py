...
class Enemy_Weapon(pygame.sprite.Sprite):
    ...
    def update(self):
        for pew in range(len(enemy_weapon_group.sprites())):
        self.rect = self.rect.move(list_coords_x[pew], list_coords_y[pew])
        if pygame.sprite.spritecollideany(self, horizontal_borders):
             pygame.sprite.spritecollide(self, enemy_weapon_group, True)
            del list_coords_x[pew]

        if pygame.sprite.spritecollideany(self, vertical_borders):
            pygame.sprite.spritecollide(self, enemy_weapon_group, True)
            del list_coords_y[pew]
            print(list_coords_x, list_coords_y)
...
while running:
    ...
    if enemy_left_group.sprites() != [] and reload_2_red % 75 == 0:
        ...
        reload_2_red = 1
    if reload_2_red % 25 == 0:
        enemy_weapon_group.update()
        print(enemy_weapon_group.sprites())
    reload_2_red += 1
